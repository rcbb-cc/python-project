...
class Bilibili_Spider():
    ...
    def __init__(self, uid, save_dir_json='json', save_by_page=False, t=2):
        print('[start Bilibili Spider初始化]')
        self.t = t
        self.uid = uid
        ...
        print('[保存文件夹<{}>]'.format(save_dir_json))
        print('[是否分页保存<{}>]'.format(save_by_page))
        print('[每页获取间隔时间<{}>]'.format(t))
        print('[Bilibili Spider初始化 end]')
    ...
    def get(self):
        ...
        while self.page_num == 0:
            print('[获取用户页面数失败，网络状况不佳，正在重试...]')
            self.page_num, self.user_name = self.get_page_num()
        print('[总页数<{}> 用户名<{}>]'.format(self.page_num, self.user_name))
        ...
        for idx in range(self.page_num):
            ...
            print('[总条数<{}>]'.format(len(urls_page)))
            print(bvs_page)
            sys.stdout.flush()
            bvs.extend(bvs_page)
